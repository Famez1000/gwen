...
class LegacySubscriptionScreen extends StatefulWidget {
  ...
  const LegacySubscriptionScreen({super.key, this.isOnboardingPaywall = false});

  @override
  State<LegacySubscriptionScreen> createState() =>
      _LegacySubscriptionScreenState();
}

class _LegacySubscriptionScreenState extends State<LegacySubscriptionScreen> {
  ...
  void _addDebugLine(String line) {
    ...
    if (kDebugMode) {
      debugPrint('[LegacySubscriptionScreen] $line');
    }
...
enum _SubscriptionPlan {
  monthly(price: '\$4.99', periodLabel: 'per month'),
  yearly(price: '\$39.99', periodLabel: 'per year');
